...
function excelDate(value) {
  ...
  if (ru) {
    return new Date(Date.UTC(Number(ru[3]) < 100 ? 2000 + Number(ru[3]) : Number(ru[3]), Number(ru[2]) - 1, Number(ru[1])));
  }
...
async function microsoftAccessToken() {
  ...
  const scope = String(process.env.MICROSOFT_SCOPE || "https://graph.microsoft.com/.default").trim();
  ...
  if (!response.ok || !payload.access_token) {
    throw new Error(payload.error_description || payload.error || `Microsoft token HTTP ${response.status}`);
  }
  return payload.access_token;
}

async function fetchWorkbookBufferViaGraph() {
  const source = String(process.env.SHAREPOINT_FILE_URL || process.env.ENROLLMENT_XLSX_URL || "").trim();
  if (!source) throw new Error("SHAREPOINT_FILE_URL is not configured");

  const accessToken = await microsoftAccessToken();
  if (!accessToken) throw new Error("Microsoft client credentials are not configured");

  const shareId = `u!${base64Url(source)}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(details || `Microsoft Graph HTTP ${response.status}`);
  }
  return Buffer.from(await response.arrayBuffer());
}

async function fetchWorkbookBufferDirect() {
  const source = String(process.env.ENROLLMENT_XLSX_URL || "").trim();
...
async function fetchWorkbook() {
  ...
  if (process.env.SHAREPOINT_FILE_URL && process.env.MICROSOFT_CLIENT_ID && process.env.MICROSOFT_CLIENT_SECRET && process.env.MICROSOFT_TENANT_ID) {
    try {
      const buffer = await fetchWorkbookBufferViaGraph();
      if (buffer.subarray(0, 2).toString() === "PK") {
        return XLSX.read(buffer, { type: "buffer", cellDates: true });
      }
      errors.push("graph returned a non-xlsx response");
    } catch (error) {
  ...
  try {
    const buffer = await fetchWorkbookBufferDirect();
    if (buffer.subarray(0, 2).toString() !== "PK") {
      throw new Error("SharePoint requires anonymous access or a service token");
    }
    return XLSX.read(buffer, { type: "buffer", cellDates: true });
  } catch (error) {
...
function parsePlan(workbook) {
  ...
  if (!items.length) throw new Error("Failed to read enrollment plan");
  return items;
}
...
function buildDashboard(workbook) {
  ...
  const byProgram = plans.map((plan) => {
    ...
    return {
      ...plan,
      actual: count,
      remaining: Math.max(0, plan.plan - count),
      progress: plan.plan ? Math.round(count / plan.plan * 1000) / 10 : 0,
    };
  }).sort((a, b) => b.actual - a.actual || b.plan - a.plan);

  const unmatched = applicants.filter((item) => !matchPlan(item.program, plans));
  if (unmatched.length) {
    byProgram.push({ program: "Другие / не сопоставлены", plan: 0, actual: unmatched.length, remaining: 0, progress: 0 });
  }
...
async function queueStatus() {
  ...
  if (!apiUrl) {
    return {
      ok: false,
      configured: false,
      adminUrl: "https://queue.mok.kz/admin",
      message: "For live queue data, add QUEUE_API_URL and QUEUE_API_TOKEN",
    };
  }
